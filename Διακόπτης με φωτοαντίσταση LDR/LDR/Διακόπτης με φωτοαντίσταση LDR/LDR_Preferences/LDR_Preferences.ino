...
// Set device as a Wi-Fi Station
  ...
  RemoteXY.edit_1 = Preferences_timiFotos ;
  RemoteXY.led_1 = 0 ;
}
...
//----------------------------------------  
  ...
  Serial.println(Preferences_timiFotos);
  Serial.println(phone);
  delay(500);

    if (RemoteXY.button_1 == 1){
         Preferences_timiFotos = RemoteXY.edit_1 ;
         RemoteXY.level_1 = ( map(timiFotos, 0, 500, 0, 100) ) ;
         RemoteXY.button_1 = 0 ;
         preferences.begin("LDR-app", false);
         preferences.putUInt("LDR-timiFotos", Preferences_timiFotos);
         phone = "+306940791159" ;
         preferences.putString("LDR-Phone", phone);
         preferences.end();
    }
      ...
      if (!RemoteXY.switch_1){
        // RemoteXY.led_1 = (timiFotos <  Preferences_timiFotos -100)?0:1;
         if (timiFotos >= Preferences_timiFotos  ){ 
             digitalWrite(RelayPin,HIGH);
             RemoteXY.led_1 = 1 ;
         }
         else if (timiFotos <  Preferences_timiFotos -100 ) {
             digitalWrite(RelayPin, LOW);
             RemoteXY.led_1 = 0 ; 
         } 
        else {
...
//-----------------------------------
void  chek_prefereces(){
//-----------------------------------  
    preferences.begin("LDR-app", false);
    Preferences_timiFotos = preferences.getUInt("LDR-timiFotos",200);
                timiFotos = preferences.getFloat("LDR-timiFotos2",0);
                phone  = preferences.getString("LDR-Phone", "+306977996927");
    preferences.end();
}
